...
function SwitchTranslation(translation)	{
	...
	if (translation == 99)	{
		...
		document.getElementById("metadata_registry_domain_uri").textContent = proposed;
		document.getElementById("metadata_registrar_domain_uri").textContent = proposed;
		...
		document.getElementById("domain_next_apex").textContent = "";
		...
		document.getElementById("metadata_registry_domain_uri").textContent=proposed+"URI naar een registry-RDAP JSON-respons; relatie 'self'.";
		document.getElementById("metadata_registrar_domain_uri").textContent=proposed+"URI naar een registrar-RDAP JSON-respons; relatie 'related'.";
		...
		document.getElementById("domain_next_apex").textContent = proposed + "Een volgende domein-apex mag onder dit domein worden geregistreerd.";
		...
		document.getElementById("domain_latest_data_mutation_at").textContent = modified + "Verschillende mutatieniveaus: domeinobject (registry) versus contactobject (registrar).";
		...
		document.getElementById("metadata_registry_domain_uri").textContent=proposed+"URI to a registry RDAP JSON response; relationship 'self'.";
		document.getElementById("metadata_registrar_domain_uri").textContent=proposed+"URI to a registrar RDAP JSON response; relationship 'related'.";
		...
		document.getElementById("domain_next_apex").textContent = proposed + "A next domain apex may be registered below this domain.";
		...
		document.getElementById("domain_latest_data_mutation_at").textContent = modified + "Distinct mutation layers: domain object (registry) vs contact object (registrar).";
		...
		document.getElementById("metadata_registry_domain_uri").textContent=proposed+"URI zu einer Registry-RDAP-JSON-Antwort; Beziehung „self“.";
		document.getElementById("metadata_registrar_domain_uri").textContent=proposed+"URI zu einer Registrar-RDAP-JSON-Antwort; Beziehung „related“.";
		...
		document.getElementById("domain_next_apex").textContent = proposed + "Ein nächster Domain-Apex darf unterhalb dieser Domain registriert werden.";
		...
		document.getElementById("domain_latest_data_mutation_at").textContent = modified + "Unterschiedliche Mutationsebenen: Domainobjekt (Registry) versus Kontaktobjekt (Registrar).";
		...
		document.getElementById("metadata_registry_domain_uri").textContent=proposed+"URI vers une réponse JSON RDAP du registre ; relation « self ».";
		document.getElementById("metadata_registrar_domain_uri").textContent=proposed+"URI vers une réponse JSON RDAP du bureau d’enregistrement ; relation « related ».";
		...
		document.getElementById("domain_next_apex").textContent = proposed + "Un prochain apex de domaine peut être enregistré sous ce domaine.";
		...
		document.getElementById("domain_latest_data_mutation_at").textContent = modified + "Niveaux de mutation distincts : objet domaine (registre) vs objet contact (registrar).";
...
$view_tld_uri = $server_uri.'/tld/index.php?language='.$viewlanguage.'&tld='.$data[$pd]['registry']['metadata']['tld_ascii_name'];;
...
$html_text .= '<tr><td id="instruction" style="font-size: 0.8rem; vertical-align:middle; text-align: right;"></td><td><form action='.htmlentities($_SERVER['PHP_SELF']).' method="get">
	...
	<a style="font-size: 0.9rem" href="https://relationships.hostingtool.nl/email_modeling" target="_blank">Email modeling</a> - <a style="font-size: 0.9rem" href="https://github.com/janwillemstegink/relationships.hostingtool.nl" target="_blank">Code/issues on GitHub</a> - <a style="font-size: 0.9rem" href="https://janwillemstegink.nl/" target="_blank">Insight at janwillemstegink.nl</a></td><td></td></tr>';
if (true or $pd == mb_strtolower($data[$pd]['registry']['domain']['ascii_name']) or empty($data[$pd]['registry']['domain']['ascii_name']))	{
	...
	$html_text .= '<tr id="207" style="display:none"><td>global_domain_uri</td><td>'.$data[$pd]['registry']['metadata']['global_domain_uri'].'</td><td id="metadata_global_domain_uri"></td><td>'.$data[$pd]['registrar']['metadata']['global_domain_uri'].'</td></tr>';
	...
	$registrar_domain_uri = str_replace('https://', '', $data[$pd]['registry']['metadata']['registrar_domain_uri']);
	...
	$html_text .= '<tr id="305" style="display:none"><td>domain_next_apex</td><td>'.$data[$pd]['registry']['domain']['next_apex'].'</td><td id="domain_next_apex"></td><td>'.$data[$pd]['registrar']['domain']['next_apex'].'</td></tr>';	
